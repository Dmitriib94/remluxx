...
export function initServicesToggle() {
  ...
  // Синхронизируем начальное состояние (thumb + тексты карточек)
  const activeButton = buttons.find((button) => button.classList.contains('is-active'));
  const initialMode = activeButton?.dataset.servicesMode || MODE_WORKS;
  setMode(initialMode);
}
...
export function initServicesSection() {
  if (servicesSectionInitialized) return;
